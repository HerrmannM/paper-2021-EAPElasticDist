    ...
    for d in datasets:
        ...
        for e in execs:
            ...
            axs.text(0.03, 0.97, dextras[d],
                     ...
                     fontdict={'family': 'monospace', 'fontsize':FONTSIZE}
                     )
    # End of for d in datasets
    handles, labels = axstab[0].get_legend_handles_labels()
    fig.set_tight_layout({"rect": (0, 0.1, 1, 1.025)})
    leg = fig.legend(handles, labels, ncol=4, loc="upper center", bbox_to_anchor=[0.5, 0.1], fontsize=FONTSIZE)
