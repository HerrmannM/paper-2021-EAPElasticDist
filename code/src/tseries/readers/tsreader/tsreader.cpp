    ...
    if (it != directive_map.end()) {
        ...
        switch (it->second) {
            ...
            case DirectiveCode::dir_series_length: {
                ...
                } else { return {"Error while reading directive @serieslength (positive integer number expected)"}; }
                state = (&TSReader::read_header);
                return {};
            }
        ...
        do {
            ...
            if (has_labels && cur_dim == ndim) {
                ...
                if (c == '\n' || c == EOF) {
                    ...
                    dataset.push_back(TSeries(std::move(series), ndim, has_missing, {buffer}));
                ...
                do {
                    ...
                    if (buffer == "?") {
                        ...
                        if (missing.empty() || missing.back() != missing_index) { missing.push_back(missing_index); }
                    } else {
                    ...
                    if (c == '\n' || c == EOF) {
                        ...
                        dataset.push_back(TSeries(std::move(series), ndim, has_missing, {}));
                        // Update min/max length
                        data.shortest_length = std::min(data.shortest_length, length);
                        data.longest_length = std::max(data.longest_length, length);
