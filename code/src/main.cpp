...
#include "distances/dtw/pruneddtw_ea.hpp"
...
void print_usage(const string& execname, ostream& out) {
  ...
  out
    << "  pru                   Pruning only, using EAP and cutoff based on the diagonal (no early abandoning!). Not for LCSS, SQED. "
    << endl;
  ...
  out
    << "  pruneddtw2018         PrunedDTW 2018 implementation (with early abandon). Only for DTW and CDTW. Not compatible with LB-Webb"
    << endl;
  out << "  eap                   Early abandoned and pruned. SQED and LCSS are only early abandoned, not pruned"
      << endl;
...
string to_string(RUNMODE rm) {
  switch (rm) {
    ...
    case PRUNEDDTW2018: return "pruneddtw2018";
    case EAP: return "eap";
      ...
      case WEBB: {
        ...
        distfun_t dfun = [innerdist, env_train, env_test, w](TRAIN_TEST, double ub) mutable {
          ...
          if (!contains(env_test, idtest)) {
            ...
            env_test.emplace(idtest,
              std::tuple(std::move(q_up), std::move(q_lo), std::move(q_lo_up), std::move(q_up_lo)));
          }
...
/** Special treatment for PrunedDTW - adapted from similarity search */
variant<string, distfun_t> do_PrunedDTW2018(const config& conf, size_t maxl, size_t minl, size_t w) {
  // Pre-check
  if (conf.dtw_lb!=NONE && minl!=maxl) {
    return {"Lower bound require same-length series"};
  }
  switch (conf.dtw_lb) {
    case NONE: {
      vector<double> cumulative(maxl, 0);
      distfun_t dfun = [w, cumulative](TRAIN_TEST, double ub) {
        return pruneddtw2018(TRAIN, TEST, cumulative.data(), w, ub);
      };
      return {dfun};
    }

    case KEOGH: {
      // Captured state and pre allocation
      vector<tuple<vector<double>, vector<double>>> env_train;
      vector<double> cumulative(maxl, 0);
      vector<double> dist_to_env(maxl, 0);

      distfun_t dfun = [maxl, w, cumulative, dist_to_env, env_train](TRAIN_TEST, double ub) mutable {
        // Compute envelopes once
        if (env_train.empty()) { compute_envelopes(train, env_train, w); }
        // Lb Keogh with dist to env
        const auto&[up, lo] = env_train[idtrain];
        double v = lb_Keogh_prunedDTW2018(TEST, up.data(), lo.data(), ub, dist_to_env.data());
        if (v<ub) {
          // Cumulative
          cumulate_prunedDTW2018(dist_to_env.data(), cumulative.data(), maxl);
          v = pruneddtw2018(TRAIN, TEST, cumulative.data(), w, ub);
        }
        return v;
      };
    ...
    case KEOGH2: {
      ...
      map<size_t, tuple<vector<double>, vector<double>>> env_test;
      vector<double> cumulative(maxl, 0);
      vector<double> dist_to_env1(maxl, 0);
      vector<double> dist_to_env2(maxl, 0);

      distfun_t dfun = [maxl, w, cumulative, dist_to_env1, dist_to_env2, env_train, env_test](TRAIN_TEST,
        double ub) mutable {
        // Compute envelopes once
        if (env_train.empty()) { compute_envelopes(train, env_train, w); }
        // Lb Keogh with dist to env
        const auto&[up, lo] = env_train[idtrain];
        double v = lb_Keogh_prunedDTW2018(TEST, up.data(), lo.data(), ub, dist_to_env1.data());
        double v1 = v;
        if (v1<ub) {
          // LB Keogh 2
          const auto lq = test[idtest].length();
          ...
          const auto&[upq, loq] = env_test[idtest];
          double v2 = lb_Keogh_prunedDTW2018(TRAIN, upq.data(), loq.data(), ub, dist_to_env2.data());
          v = v2;
          // Distance
          if (v<ub) {
            // Cumulative: pick largest
            if (v1>v2) { cumulate_prunedDTW2018(dist_to_env1.data(), cumulative.data(), maxl); }
            else { cumulate_prunedDTW2018(dist_to_env2.data(), cumulative.data(), maxl); }
            v = pruneddtw2018(TRAIN, TEST, cumulative.data(), w, ub);
          }
        }
        return v;
      ...
      // --- --- ---
    case WEBB: { return "PrunedDTW2018 is not compatible with LB-Webb"; }
  }
  return {"Should not happen at " __FILE__ ": " STRINGIFY(__LINE__)};
}
...
variant<string, distfun_t> get_distfun(const config& conf, size_t maxl, size_t minl) {
  ...
  if (conf.distance_name=="dtw") {
    switch (conf.runmode) {
      ...
      case PRUNEDDTW2018: {
        size_t w = conf.cdtw_wratio*maxl;
        return do_PrunedDTW2018(conf, maxl, minl, w);
      }
      case EAP: {
    ...
    switch (conf.runmode) {
      ...
      case PRUNEDDTW2018: { return do_PrunedDTW2018(conf, maxl, minl, w); }
      case EAP: {
    ...
    switch (conf.runmode) {
      case BASE: {
        dfun = [](TRAIN_TEST, [[maybe_unused]]double ub) { return elementwise(TRAIN, TEST); };
        break;
      }
      case EAP: {
        dfun = [](TRAIN_TEST, double ub) { return elementwise<false>(TRAIN, TEST, ub); };
        break;
      }
      case EAP_LA: {
        dfun = [](TRAIN_TEST, double ub) { return elementwise<true>(TRAIN, TEST, ub); };
        break;
      }
      default: { return {conf.distance_name.value()+" distance not compatible with "+to_string(conf.runmode)}; }
    }
...
int main(int argc, char** argv) {
  ...
  // --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
  {
    ...
    while (i<argc) {
        ...
        if (i<argc) {
          ...
          if (distname=="dtw") {
            ...
            if (i+1<argc) {
              ...
              wr = as_double(arg);
              if (epsilon && 0<=epsilon.value() && wr
                && ((!conf.lcss_int && 0<=wr.value() && wr.value()<=1) || (0<=wr.value()))) {
      ...
      else if (arg=="pruneddtw2018") { conf.runmode = PRUNEDDTW2018; }
