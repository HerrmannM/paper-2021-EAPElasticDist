...
static void get_envelopes(
    ...
    if(w==0){ for(size_t i=0; i<length; ++i) { upper[i] = lower[i] = series[i]; } return; }
    ...
    for(size_t i{1}; i < w; ++i){
        ...
        if(prev<=si){ do{up.pop_back();} while(!up.empty() && series[up.back()] <= si); } // (1) Remove while si is larger than up[back]
        else { do{lo.pop_back();} while(!lo.empty() && series[lo.back()] >= si); }        // (2) Remove while si is smaller than lo[back]
        up.push_back(i);
        lo.push_back(i);
    }
    ...
    for(size_t i{0}; i<length-w; ++i) {
        ...
        if(prev <= si){
            ...
            if(up.empty()){ up_front_idx = idx; up_front_val = series[up_front_idx]; }
        }
        else{
            do{lo.pop_back();} while(!lo.empty() && series[lo.back()] >= si);
            if(lo.empty()){ lo_front_idx = idx; lo_front_val = series[lo_front_idx]; }
        }
        ...
        // 3) trim the front
        if (up_front_idx + w <= i) { up.pop_front(); up_front_idx = up.front(); up_front_val = series[up_front_idx]; }
        if (lo_front_idx + w <= i) { lo.pop_front(); lo_front_idx = lo.front(); lo_front_val = series[lo_front_idx]; }
    }
 ...
 * @param w The window for which the envelope is computed.
 */
inline void get_envelopes(const std::vector<double>& series,
...
static void get_up_envelope(
    ...
    if(length == 0) { return; }
    if(w>=length){ w=length-1; }
    if(w==0){ for(size_t i=0; i<length; ++i) { upper[i] = series[i]; } return; }
 ...
 * @param w The window for which the envelope is computed.
 */
inline void get_up_envelope(const std::vector<double>& series,
...
static void get_lo_envelope(
    ...
    if(length == 0) { return; }
    if(w>=length){ w=length-1; }
    if(w==0){ for(size_t i=0; i<length; ++i) { lower[i] = series[i]; } return; }
 ...
 * @param w The window for which the envelope is computed.
 */
inline void get_lo_envelope(const std::vector<double>& series,
