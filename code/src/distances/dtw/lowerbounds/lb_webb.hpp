...
static void get_envelopes_Webb(
        const double* series, size_t length,
        double *upper, double *lower,
        double *lower_upper, double *upper_lower,
        size_t w
){
    get_envelopes(series, length, upper, lower, w);
    get_lo_envelope(upper, length, lower_upper, w);
    get_up_envelope(lower, length, upper_lower, w);
}

/** Create envelopes for lb Webb - vector version. Reallocation may occur! */
inline void get_envelopes_Webb(
        const std::vector<double>& series,
        std::vector<double>& upper, std::vector<double>& lower,
        std::vector<double>& lower_upper, std::vector<double>& upper_lower,
        size_t w){
    get_envelopes(series, upper, lower, w);
    get_lo_envelope(upper, lower_upper, w);
    get_up_envelope(lower, upper_lower, w);
}
